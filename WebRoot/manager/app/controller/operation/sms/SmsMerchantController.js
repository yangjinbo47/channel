Ext.define('CMS.controller.operation.sms.SmsMerchantController', {
	extend : 'Ext.app.Controller',
	views : ['CMS.view.operation.sms.SmsMerchantManager'],
	stores : ['CMS.store.operation.sms.SmsMerchantStore'],
	refs : [{
		ref : 'smsMerchantManagerView',
		selector : 'smsMerchantManagerView'
	}],
	init : function() {
		this.control({
			'smsMerchantManagerView' : {
				render : function(grid) {
					var roweditplugin = grid.getPlugin('smsMerchantManager-rowediting');
					roweditplugin.on({
			...
			'smsMerchantManagerView > toolbar > button[name=add]' : {
				click : this.onAddClick
			},
			'smsMerchantManagerView > toolbar > button[name=delete]' : {
				click : this.onDelClick
			},
			'smsMerchantManagerView > toolbar > button[name=search]' : {
				click : this.search
			},
			'smsMerchantManagerView > toolbar > button[name=reset]' : {
				click : this.reset
			}
		});
	},
	...
	onAddClick : function() {
		var grid = this.getSmsMerchantManagerView();
		var roweditplugin = grid.getPlugin('smsMerchantManager-rowediting');
		roweditplugin.cancelEdit();
		var r = Ext.create('CMS.model.operation.sms.SmsMerchantModel', {
            merchantName: '商户名',
            email:'邮箱',
            contact:'联系人',
            telephone:'联系电话',
		...
		if (!rs.length) {
			...
			Ext.Msg.confirm("请确认", "是否真的要删除指定的内容", function(button, text) {
				if (button == "yes") {
					...
					ids = ids.slice(0, -1);
					Ext.Ajax.request({
						url : '../operation/smsMerchant_delete.action',
						params : {
							'ids' : ids
						},
	...
	roweditupdate : function(plugin) {
		...
		Ext.each(grid.getSelectionModel().getSelection(), function(row, index, value) {
			...
			Ext.Ajax.request({
				url : '../operation/smsMerchant_save.action',
				params : {
					'id' : id,
	...
	search : function() {
		var grid = this.getSmsMerchantManagerView();
		var queryParams = {};
		queryParams.merchantName = this.getSmsMerchantManagerView().down('toolbar > textfield[name=merchantName]').getValue();
