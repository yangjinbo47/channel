Ext.define('CMS.view.operation.sms.SmsOrderManager', {
	extend : 'Ext.panel.Panel',
	xtype : 'smsOrderManagerView',
	layout : 'border',
...
//    }],
	initComponent : function(){
		...
		this.westPanel = Ext.create('CMS.extensions.FilterTreePanel',{
			...
			hideHeaders : true,
			useArrows : true,
	 		store : 'CMS.store.operation.sms.SmsSellerTreeStore',
	 		columns : [{
                xtype: 'treecolumn',
                header : '渠道名称',
                hideable : false,
		...
		this.centerPanel = Ext.create('Ext.grid.Panel',{
	 		...
	 		xtype : 'gridpanel',
	 		padding : '0 5 0 5',
	 		store : 'CMS.store.operation.sms.SmsOrderStore',
		 	columns : [{
		 		header : '订单号',
				dataIndex : 'orderId',
				flex : 2
			...
			tbar : ['起始时间',{
				...
				validator : function(){
					var rawValue = this.getRawValue(), endField = Ext.getCmp('smsorder_endtime'),endRawValue = endField.getRawValue();
				...
				validateOnChange : false,
				validator : function(){
					var rawValue = this.getRawValue(), startField = Ext.getCmp('smsorder_starttime'),startRawValue = startField.getRawValue();
