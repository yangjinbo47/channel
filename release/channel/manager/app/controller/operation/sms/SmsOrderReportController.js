Ext.define('CMS.controller.operation.sms.SmsOrderReportController', {
	extend : 'Ext.app.Controller',
	views : ['CMS.view.operation.sms.SmsOrderReportManager'],
	stores : ['CMS.store.operation.sms.SmsOrderReportStore'],
	refs : [{
		ref : 'smsOrderReportView',
		selector : 'smsOrderReportView'
	}],
	init : function() {
		this.control({
			'smsOrderReportView > toolbar > button[name=report]' : {
				click : this.onReportClick
			},
			'smsOrderReportView > toolbar > button[name=total]' : {//合计按钮
				click : this.onTotalClick
			},
			'smsOrderReportView actioncolumn' : {//统计按钮
	...
	onReportClick : function() {
		var me = this,grid = me.getSmsOrderReportView();
...
//		grid.getStore().loadPage(1);
	...
	detail : function(grid, rowIndex, colIndex) {
		...
		var me = this,gd = me.getSmsOrderReportView();
...
//		var endTimeHiddenField = gd.down('toolbar > hidden[name=endTime]');
		...
		var win = Ext.create('CMS.view.operation.sms.SmsOrderReportDetailWindow');
		var panel = Ext.create('CMS.view.operation.sms.SmsOrderReportDetailPanel');
		with(panel.form){
	...
	onTotalClick : function() {
		var grid = this.getSmsOrderReportView();
		...
		if (!rs.length) {
			...
			var win = Ext.create('Ext.window.Window',{
				id:'smsOrderReportTotalWin',
				title:'合计',
				height:200,
	      		width:900
			});
			var panel = Ext.create('Ext.grid.Panel',{
				id : 'smsOrderReportTotalPanel',
				layout : 'fit',
				store : store,
				columns : [{
					header : '名称',
