...
public class SendOpenHisToBJ implements Runnable{

	private static Log visitLog = LogFactory.getLog("visitLog");
	
	private TOpenOrderHistory tOpenOrderHistory;
	
	public SendOpenHisToBJ(TOpenOrderHistory tOpenOrderHistory) {
		this.tOpenOrderHistory = tOpenOrderHistory;
	}
	
	@Override
	public void run() {
		try {
			String orderId = tOpenOrderHistory.getOrderId();
			Integer sellerId = tOpenOrderHistory.getSellerId();
			Integer appId = tOpenOrderHistory.getAppId();
			
			Integer price = tOpenOrderHistory.getFee();
			String gatewayCode = appId+"-"+price;
			String productCode = sellerId+"-"+appId+"-"+price;
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
			Date payTime = tOpenOrderHistory.getPayTime();
			if (Utils.isEmpty(payTime)) {
				payTime = new Date();
			}
			String paymentTime = sdf.format(payTime);
			String payPhone = tOpenOrderHistory.getPayPhone();
			String province = tOpenOrderHistory.getProvince();
